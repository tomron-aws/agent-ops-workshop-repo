import traceback
import pandas as pd
import requests, ftplib, io, json, datetime
...
def get_world_indices(headers = {'User-agent': 'Mozilla/5.0'}):
    '''Returns the world_indices table from Yahoo Finance'''
    
    site = "https://finance.yahoo.com/markets/world-indices"
    tables = pd.read_html(requests.get(site, headers=headers).text)
...
def get_bonds(headers = {'User-agent': 'Mozilla/5.0'}):
    
    '''Returns the bonds table from Yahoo Finance'''
    
    site = "https://finance.yahoo.com/markets/bonds/"
    tables = pd.read_html(requests.get(site, headers=headers).text)
